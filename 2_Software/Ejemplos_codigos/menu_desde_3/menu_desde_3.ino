...
void loop() {
  ...
  for(i=0;i<4;i++)
  {
    for(j=0;j<PROM_INTERVAL;j++)
    {
      lectura[i] = analogRead(i); //Leemos el pin analógico Ai  
      valorAcumulado += lectura[i];
    }
    lectura[i] = promediador(valorAcumulado);
    valorAcumulado = 0;
    }
...
float promediador(float valorAcumulado)
{
  return valorAcumulado / PROM_INTERVAL;
}
